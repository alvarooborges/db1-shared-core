...
public class UpdatePunishmentStartedAtSpec extends UpdateSqlSpec<Void> {
    ...
    public PreparedStatementCreator getPreparedStatementCreator() {
        ...
        return (Connection connection) -> {
            ...
            PreparedStatement statement = connection.prepareStatement(String.format(
                    "UPDATE `%s` SET `started_at` = ? WHERE `id` = ?",
                    CoreConstants.Databases.Mysql.Tables.PUNISHMENTS_TABLE_NAME
            ), Statement.RETURN_GENERATED_KEYS);

            statement.setTimestamp(1, new Timestamp(this.punishment.getStartedAt().getTime()));
